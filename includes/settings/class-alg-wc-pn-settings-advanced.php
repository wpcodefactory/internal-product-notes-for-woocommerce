...
if ( ! class_exists( 'Alg_WC_PN_Settings_Advanced' ) ) :

	/**
	 * Alg_WC_PN_Settings_Advanced class.
	 *
	 * @version 3.2.0
	 * @since   2.6.0
	 */
	class Alg_WC_PN_Settings_Advanced {
			...
			foreach ( $options as $option => $option_title ) {
				...
				foreach ( $types as $type => $type_title ) {
					...
					foreach ( array( 'private', 'public' ) as $private_or_public ) {
						$opt_suffix  = ( in_array( $type, array( 'single', 'loop' ), true ) ? '_frontend_enabled' : '' );
						...
						if (
							in_array( $type, array( 'single', 'loop' ), true ) &&
							'content' === $option
						) {
							continue;
						}
						...
						$id        = (
							'product_tab' === $type ?
							"alg_wc_pn_{$private_or_public}_product_tab_options" :
							"alg_wc_pn_{$private_or_public}_{$type}_formatting_options"
						);
						$_settings = array(
						...
						if ( 'glue' === $option ) {
							...
							$_settings['desc_tip'] = sprintf(
								/* translators: %s: placeholder */
								__( 'Placeholder: %s', 'product-notes-for-woocommerce' ),
								'<em>%product_notes%</em>'
							);
						} else {
							$_settings['checkboxgroup'] = (
								1 === $i ?
								'start' :
								(
									count( $options ) * count( $types ) * 2 === $i ?
									'end' :
									''
								)
							);
...
return new Alg_WC_PN_Settings_Advanced();
